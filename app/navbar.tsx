...
const Navbar = () => {
  ...
  return (
    <AnimatePresence>
      <motion.nav
        key="navbar"
        className="fixed top-0 left-0 right-0 bg-[#4542429c] z-50 py-4 px-6 md:px-10 border-b"
        initial="initial"
      ...
      >
        <div className="mx-auto flex justify-between items-center">
          <Link href={"/"} className="flex items-center space-x-1">
            ...
            <span className={`text-xl font-bold ${mobileMenuOpen ? 'text-white' : ''}`}>HustleHub</span>
          </Link>
          ...
          <div className="hidden md:flex space-x-8 absolute left-1/2 transform -translate-x-1/2">
            {links.map((link) => (
              <Link
                ...
                key={link.href}
                className={`${
                  pathname === link.href ? "text-black" : "text-neutral-100"
